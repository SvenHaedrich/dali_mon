...
class ForwardFrame16Bit:
    ...
    def GearColourCommand(self, opcode):
        code_dictionary = {
    ...
    def SpecialCommand(self, address_byte, opcode_byte, device_type):
        # see iec 62386-102 11.2
    ...
    def __init__(self, frame, device_type=DeviceTypes.NONE):
        ...
        if (address_byte >= 0x00) and (address_byte <= 0x7F):
            ...
            self.command_string = '         ' + self.SpecialCommand(address_byte, opcode_byte, device_type)
        ...
        if (not self.dapc) and (self.addressing != DALIAddressing.SPECIAL_COMMAND):
            self.command_string = self.GearCommand(opcode_byte)
            if device_type==DeviceTypes.COLOUR:
                self.command_string = self.GearColourCommand(opcode_byte)


class ForwardFrame24Bit:

    def DeviceCommand(self, opcode):
        # see iec 62386-102 11.2
        code_dictionary = {
    ...
    def DeviceSpecialCommand (self, address_byte,instance_byte,opcode_byte):
    ...
    def __init__(self,frame):
        ...
        if (address_byte >= 0x00) and (address_byte <= 0x7F):
            ...
            short_address = address_byte >> 1
            self.address_string = 'A{:02}      '.format(short_address)
            self.command_string = self.DeviceCommand(opcode_byte)
            return
        ...
        if address_byte == 0xFD:
            ...
            self.addressing = DALIAddressing.SPECIAL_COMMAND
            self.command_string = '         ' + self.DeviceSpecialCommand(address_byte, instance_byte, opcode_byte)
...
class ForwardFrame25Bit:
    ...
    def __init__(self,frame):
        ...
        self.command_string = self.eDALICommand(opcode_byte)
